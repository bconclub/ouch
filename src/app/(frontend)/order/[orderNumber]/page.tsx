      ...
      <div className="rounded-lg border border-line bg-surface p-6">
        ...
        <ul className="divide-y divide-line text-sm">
          {order.items.map((item, i) => (
            <li className="flex justify-between gap-3 py-2.5" key={item.id ?? i}>
              <span>
                {item.title}
        ...
        <div className="mt-4 flex items-baseline justify-between border-t border-line pt-4">
          ...
          <span className="font-display text-xl font-bold">{formatPrice(order.total)}</span>
        </div>
      </div>

      <div className="mt-8 text-center">
        <p className="mb-4 text-sm text-muted">
          One last step — send us your order on WhatsApp to confirm it. Payment and shipping are
          arranged in chat.
        </p>
        <a
          className="inline-flex items-center gap-3 bg-[#25D366] px-8 py-4 text-sm font-bold tracking-widest text-black uppercase transition-opacity hover:opacity-90"
          href={whatsappUrl}
